...
def compose_args_for_converse_api(
        ...
        if message.role not in ["system", "instruction"]:
            ...
            for c in message.content:
                if c.content_type == "text":
                    if use_guardrails:
                        # Guardrails for text content
                        # Only use guardrail for the last message
    ...
    if use_guardrails:
        ...
        # We don't need to guardrail the system instruction as they are not generated by the user
        args["system"].append({
